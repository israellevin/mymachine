...
mkchroot() {
    ...
    mkdir -p ./lib/modules
    cp -au /lib/modules/"$(uname -r)" ./lib/modules/.
    systemd-firsboot --root . --hostname="$(hostname)" --copy
}
...
mount -t tmpfs -o size=8G tmpfs "$new_root_path" || emergency Could not mount tmpfs for new root

info Getting mkma storage
mount -t proc proc /proc || emergency Could not mount proc
...
mkinitramfs() {
    ...
    cp --preserve=all --parents /lib/modules/"$(uname -r)"/modules.dep .
...
bootinitramfs() {
    ...
    local root_image="$3"
    local ramdisk_size="$4"
    local qemu_disk="${5:-./qemu_disk.img}"

    if [ ! -f "$qemu_disk" ]; then
        qemu-img create -f raw "$qemu_disk" 1G
...
mkcd() {
    mkdir -p "$1"
    cd "$1" || {
        echo Error: Could not access directory "'$1'".
        exit 1
    }
}
...
mkma() {
    ...
    bootinitramfs /boot/vmlinuz-$(uname -r) "$initramfs_image" "$root_image" 8192
